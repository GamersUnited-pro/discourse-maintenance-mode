...
2. Under the `hooks:` → `after_code:` → `cmd:` section (where `docker_manager` is), add:
   ```bash
   - git clone --branch v1.0.28 https://github.com/GamersUnited-pro/discourse-maintenance-mode.git
   ```
...
3. Rebuild your container:
   ```bash
   echo "- git clone --branch v1.0.28 https://github.com/GamersUnited-pro/discourse-maintenance-mode.git" >> containers/app.yml && ./launcher rebuild app
   ```
